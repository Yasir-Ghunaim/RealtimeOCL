...
# This is a modified version of the class Accuracy.
# Changes:
# -- Display accuracy as a percentage. Changes are in Lines {129, 131}
...
class Accuracy(Metric[float]):
    ...
    def result(self, task_label=None) -> Dict[int, float]:
        ...
        if task_label is None:
            return {k: v.result()*100.0 for k, v in self._mean_accuracy.items()}
        else:
            return {task_label: self._mean_accuracy[task_label].result()*100.0}
